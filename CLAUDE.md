...
**Private / personal skills.** A user can add their own skills without editing the public tree: any `profile/skills/*/SKILL.md` is an additional skill, equal in standing to the shipped `skills/`. At session start, read the name/description frontmatter of each `profile/skills/*/SKILL.md` that exists so you know what they do and when to invoke them, and route to them the same way you route to public skills. They live under the gitignored `profile/`, so they're personal and survive `git pull` — adding one never conflicts on upgrade. A private skill may read the public shared files (`skills/shared/*`), so it inherits the voice rules, the drafting protocol, the voice bank, and editing-intent for free. If `profile/skills/` does not exist, there are simply no private skills; carry on.
