...
## Step 1: Read config and decide whether there's a site
...
**If it succeeds and returns non-trivial content** (>500 chars, structured prose, not a 404 page): go to Step 4 with this content.

**If it fails or returns junk** (or `portfolio.llms_txt` was empty and the default `/llms.txt` 404s): fall through to Step 3.

## Step 3: Scrape the main site
...
## Step 4: Distill into structured markdown
...
Fill each section from the fetched content. Be faithful to what's on the site — don't invent. If a section has no source material on the site (e.g., "What I'm looking for" / "What I'm NOT looking for" / "Field leveling" are hand-maintained and not published anywhere on the portfolio), **do not blank it** — leave it as a gap for now and let Step 5 carry over whatever the existing file holds. Some sections are user-maintained by design; a fresh scrape is not authoritative over them.

## Step 5: Merge with the existing summary, and confirm meaningful changes (if any)
...
Write the **merged** content from Step 5 to `profile/persona.md` (the result of carry-overs plus any confirmation choices the user made) — not the raw distill from Step 4. On a first run, write the new content directly.
...
- `portfolio.url` is empty in config — there's nothing to fetch (Step 1 already handles this).
...
Never overwrite a known-good `persona.md` with degraded content. Better to keep the old summary than corrupt the source of truth for fit assessment. The same caution applies section-by-section: hand-maintained sections (commonly "What I'm looking for" / "What I'm NOT looking for" / "Field leveling") and any content that the refresh would drop or shrink are carried over by default, and meaningful losses are confirmed with the user before writing (Step 5) — never silently.
