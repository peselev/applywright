...
2. **Process it in auto mode.** Invoke the **process-job** skill with this URL and **mode = auto**. Process-job will:
   ...
   - scan, assess fit,
   - auto-proceed (Match ≥ 6) → CV + PDF + tracker row `To apply`, or
   - auto-skip (Match ≤ 5) → tracker row `Decided against applying`,
   ...
   - **Proceeded** → after process-job finishes, remove the URL and tally:
     ...
     ```
     `proceeded += 1`. Record `✓ {company} — {role} — proceed (Match {M}/10 · Appeal {A}/10)`.
...
- **Manual mode does not apply here.** Every job in the queue runs auto. If the user wants to weigh in on a borderline (Stretch) job, they run that one URL on its own and ask for manual mode.
